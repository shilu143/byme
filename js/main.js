...
function onMintermChange(appendComma = false) {
    ...
    if (computable) {
        kmp.setMinterms(minterms);
        kmp.setMaxterms(maxterms);
        kmp.setDontCares(dontCares);
...
function setReturnVariableString(str) {
    ...
    kmp.setReturnName(str);
}

function setSolutionStrings() {
    if (sop) {
        setSolution("#generic-solution", kmp.getSOPGeneric());
    }
}
...
function setAllVariables(vn, rn, dim, minterms, maxterms, dc) {
    kmp.setVariableNames(vn);
    kmp.setReturnName(rn);
    kmp.setDimension(dim);
    kmp.setMinterms(minterms);
    kmp.setMaxterms(maxterms);
    kmp.setDontCares(dc);
}
...
function calculate() {
    if (sop) {
        kmp.calculateSOPEssentials();
        setSolutionStrings();

        return kmp.getSOPEssentials();
    }
}
...
function populateKmap(minterms, dontCares, dimension) {
    ...
    dontCares.filter(v => (prevDontCares.indexOf(v) == -1)).forEach(v => setCell(v, "x"));
